...
resource "aws_glue_job" "load_mars_job" {
  name              = "${local.name-prefix}-load-mars-job"
  role_arn          = aws_iam_role.glue_service_role.arn
  glue_version      = "5.0"
  number_of_workers = 2
  worker_type       = "G.1X"
  ...
  command {
    script_location = "s3://${aws_s3_bucket.nasa_pipeline_code.bucket}/${aws_s3_object.load_mars_script.key}"
    python_version  = "3"
  }
